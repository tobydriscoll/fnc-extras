...
for k = 1:5
    plot([x y],[y y],'-'),  x = y;    % let y become the new x
    y = g(x);  plot([x x],[x y],'-')  % let g(x) become the new y
end
...
%%
% Now let's try to find the other fixed point in the same way. We'll guess
% from the graph that 1.3 is a good approximation to start with. 
cla
fplot(g,[1 2])
