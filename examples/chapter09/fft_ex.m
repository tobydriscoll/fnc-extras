...
%%
% We perform the Fourier analysis using |fft| and then examine the
% coefficients.
c = fft(y)/N;
...
% of them in general, truncated for computational use). 
f = @(x) exp( sin(pi*x) );
c = fft(f(t))/N;
semilogy(k,abs(c),'.')
...
% magnitude as a function of the frequencies. This decay rate is directly
